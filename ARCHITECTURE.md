...
启动不读取安全存储。基础监控启动后，各采集循环独立节流。额度采集统一等待用户主动解锁：Codex、Cursor、Clash 与 API 连接器在解锁前不读取、不展示；解锁后秘密进入本次运行内存并启动额度刷新。退出时停止任务、播放器、观察者并清空内存凭据。
...
远程设备持久化 `networkScope / pinned / sortOrder`。监控同时最多启动 3 个 SSH 采集；局域网/VPN 条件不满足时进入 `expectedOffline`，暂停探测与飞书告警。局域网证据拒绝默认路由及其 `WASCLONED/CLONING` 主机派生项，仅接受同子网、真实专用物理路由或有界 TCP 可达证据；NWPathMonitor 观察到路径变化后宽限 90 秒，连续 3 次失败才生成异常事件。SSH stdout/stderr 在进程运行期间并发排空并限长，解析器拒绝超限数值与截断结构。
